...
def dxf_export_mesh_file
  ...
  if (export_ents.length > 0)
    ...
    #exported file name
    out_name = UI.savepanel(file_type.upcase + " file location", "" , "#{File.basename(model.path).split(".")[0]}." + file_type)
    if out_name
    ...
    def dxf_write_face(face, tform, layername)
      ...
      def dxf_write_stl(face, tform)
        ...
        def dxf_dxf_options_dialog
          options_list = ["polylines", "polyface mesh", "triangular mesh", "lines", "stl"].join("|")
        ...
        def dxf_dxf_units_dialog
          ...
          model_unit = units[Sketchup.active_model.options['UnitsOptions']["LengthUnit"]]
          export_unit = units[2] # model_unit
          units_list = units.join("|")
          prompts = ["Export unit: "]
          enums = [units_list]
          values = [export_unit]
        ...
        if (not $SKETCHUP_STL_LOADED)
          ...
          if Sketchup::Menu.instance_method(:add_item).arity == 1
            UI.menu('File').add_item("Export to DXF/STL...") { dxf_export_mesh_file }
            UI.menu("Tools").add_item("Export to DXF/STL...") { dxf_export_mesh_file }
          else
            UI.menu('File').add_item("Export to DXF/STL...", insert_index) { dxf_export_mesh_file }
            UI.menu("Tools").add_item("Export to DXF/STL...", insert_index) { dxf_export_mesh_file }
          end
        end
